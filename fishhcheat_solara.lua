...
local NoSniperBeam = false
local NoFallDamage = false
local NoUndisguise = false
local NoSelfDamage = false

local Storage = Instance.new("Folder")
Storage.Name = tostring(math.random(1000, 9999))
...
-- Since solara has no hooks, we have to resort to this very unstable method of just replacing remotes with fake ones and hoping it doesn't brick the game
Toggles.NoSniperBeam:OnChanged(function()
	if Toggles.NoSniperBeam.Value ~= NoSniperBeam then
		local SniperBeam = RepStorage.Events:FindFirstChild("ReplicateDot")
		NoSniperBeam = Toggles.NoSniperBeam.Value
		if Toggles.NoSniperBeam.Value then
			if SniperBeam then
				local NewSniperBeam = Instance.new("RemoteEvent")
				NewSniperBeam.Name = "ReplicateDot"
				NewSniperBeam.Parent = RepStorage.Events
				SniperBeam.Parent = Storage
			end
		else
			if Storage:FindFirstChild("ReplicateDot") then
				RepStorage.Events.ReplicateDot:Destroy()
				Storage.ReplicateDot.Parent = RepStorage.Events
			end
		end
	end	
end)
...
Toggles.NoFallDamage:OnChanged(function()
	if Toggles.NoFallDamage.Value ~= NoFallDamage then
		NoFallDamage = Value
		local FallDamage = RepStorage.Events:FindFirstChild("FallDamage")
		if Value then
			if Toggles.NoFallDamage.Value then				
				local NewFallDamage = Instance.new("RemoteEvent")
				NewFallDamage.Name = "FallDamage"
				NewFallDamage.Parent = RepStorage.Events
				FallDamage.Parent = Storage
			end
		else				
			if Storage:FindFirstChild("FallDamage") then
				RepStorage.Events.FallDamage:Destroy()
				Storage.FallDamage.Parent = RepStorage.Events
			end
		end
	end
end)

Toggles.NoUndisguise:OnChanged(function()
	if Toggles.NoUndisguise.Value ~= NoUndisguise then
		NoUndisguise = Value
		local Undisguise = RepStorage.Events:FindFirstChild("Undisguise")
		if Value then
			if Undisguise then
				local NewUndisguise = Instance.new("RemoteEvent")
				NewUndisguise.Name = "Undisguise"
				NewUndisguise.Parent = RepStorage.Events
				Undisguise.Parent = Storage
			end
		else
			if Storage:FindFirstChild("Undisguise") then
				RepStorage.Events.Undisguise:Destroy()
				Storage.Undisguise.Parent = RepStorage.Events
			end
		end
	end
end)

Toggles.NoSelfDamage:OnChanged(function()
	if Toggles.NoSelfDamage.Value ~= NoSelfDamage then
		NoSelfDamage = Value
		local SelfDMG = RepStorage.Events:FindFirstChild("IMISSED")
		if Value then
			if SelfDMG then				
				local NewSelfDMG = Instance.new("RemoteEvent")
				NewSelfDMG.Name = "IMISSED"
				NewSelfDMG.Parent = RepStorage.Events
				SelfDMG.Parent = Storage
			end
		else					
			if Storage:FindFirstChild("IMISSED") then
				RepStorage.Events.IMISSED:Destroy()
				Storage.IMISSED.Parent = RepStorage.Events
			end
		end
	end
end)
...
GB_Fun:AddToggle('OneLife', {Text = 'One Life', Default = false, Tooltip = 'Upon death, you get sent to hell (literally)'})
...
function AddBoxHandleAdorn(projectile)
	if not Toggles.Chams.Value then return end
...
function Visuals(Player)
    ...
    RunService:BindToRenderStep(Player.Name .. 'Esp', 1, function()
        if Player.Character and (Toggles.ESPTeammates.Value or Player.Team ~= LocalPlayer.Team) then -- Player and 
            local base = Player.Character
			local plrWeapon = base:GetAttribute("EquippedWeapon") or 'None'
            ...
            if (base and Head and Root and base:GetAttribute("isAlive") == true) then
                ...
                --if top_isrendered or bottom_isrendered then -- move this up maybe?
					...
					if buildingcham then
						if Toggles.ChamsTeamCheck.Value then
							if v.TeamColor3.BrickColor == BrickColor.new("Bright red") then
								...
								if Target and Target.Parent == v then
...
local GetClosestToMouse = function(TargetPart)
	...
	if not (Map and Map:FindFirstChild("Ignore")) then return ToRet,Closest end
